...
- generating an initial task backlog from planning documents, when one does not yet exist
...
- Generating a task backlog from planning documents
...
- Local CLI execution
- AI-assisted task backlog generation
...
- Project documentation has been created (PRD, Architecture, AI Engineering Spec).
- Tasks are already defined, or can be generated by ADO from that documentation (see FR-013).
...
ADO is not responsible for generating missing project documentation (PRD, Architecture, AI Engineering Spec); it only generates the task backlog derived from documentation that already exists.
...
## FR-013 Task Backlog Generation

ADO shall be able to generate `docs/TASKS.md` from a repository's planning documents.
...
The generated backlog shall:

- follow the same grammar the Story Planner parses
- be validated before being written to disk
- never overwrite an existing backlog with different content unless explicitly forced

Generation shall be cached by a hash of the input documents so that unchanged documents reproduce identical output without re-invoking the AI agent.
...
## NFR-007 Idempotent Task Generation

Given unchanged planning documents, task backlog generation shall produce identical output across runs.

Re-invoking the AI agent for unchanged input is unnecessary and shall be avoided.
