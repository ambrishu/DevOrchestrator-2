...
class RunReviewCommandTest : FunSpec({
    ...
    test("reviews an explicitly given story ID and exits 0 with no blocking issues") {
        every { contextBuilder.buildContext(story, any<Path>()) } returns context
        every { codeReviewAgent.review(context, any<Path>()) } returns ReviewResult()

        val result = RunReviewCommand().test(listOf("ADO-002", "--path", "/repo"))

        result.statusCode shouldBe 0
        result.output shouldContain "Blocking Issues (0):"
    }

    test("exits 1 when the review finds a blocking issue") {
        every { contextBuilder.buildContext(story, any<Path>()) } returns context
        every { codeReviewAgent.review(context, any<Path>()) } returns
            ReviewResult(blockingIssues = listOf("Thread-unsafe access"))

        val result = RunReviewCommand().test(listOf("ADO-002", "--path", "/repo"))

        result.statusCode shouldBe 1
        result.output shouldContain "Thread-unsafe access"
    }
    ...
    test("falls back to the next executable story when no ID is given") {
        ...
        val result = RunReviewCommand().test(listOf("--path", "/repo"))
    ...
    test("exits 1 when the given story ID does not exist") {
        val result = RunReviewCommand().test(listOf("ADO-999", "--path", "/repo"))
    ...
    test("exits 1 and reports the error when the review invocation fails") {
        every { contextBuilder.buildContext(story, any<Path>()) } returns context
        every { codeReviewAgent.review(context, any<Path>()) } throws ReviewFailureException("claude not found")
